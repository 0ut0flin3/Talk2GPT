...
**For Windows Users: on Windows is partially fixed an installation bug (on Windows) related to PyGame. now pygame WON'T be automatically installed when you install GPTalk, this allows you to install gptalk without problems and use text mode, THEN you can install pygame separately ,by yourself,if you want to use speech on Windows. If you have not PyGame installed it will cause an error (windows only) when you try to use speech mode. Work in progress to fix this issue. LINUX/MAC ARE NOT AFFECTED. PYGAME IS NOT REQUIRED FOR LINUX/MAC and speech mode on Linux works well**
